...
#ifdef Factory_SaveReport
static int read_test_report(item_t *items, FILE *fp)
{
...
static int create_report(item_t *item, item_t *rpt_items, int maxitems, char *buf, int size)
{
...
#ifdef Factory_SaveReport  
		//	if((mod->test_result==1)||(mod->test_result==2))	  
				{
            ptr = buf + len;
            len += sprintf(ptr, "[%c] %s ",
	...
	if(fp == NULL)
	{
	    LOGD(TAG "create_report TEST_REPORT_SAVE_FILE is null");
	}
	else
	{
	    LOGD(TAG "create_report TEST_REPORT_SAVE_FILE is not null");
...
#ifdef Factory_SaveReport 
static int create_report_open(item_t *item, item_t *rpt_items, int maxitems, char *buf, int size)
{
    struct ftm_module *mod;
    int i = 0, len = 0,itemNum=0,readitemNum=0;
    char *ptr = buf;
    char result[] = { ' ', 'O', 'X' };
    color_t bgc[] = { 0, COLOR_GREEN, COLOR_RED };
    //handle of testreport.log
	FILE *fp = NULL;
	char ReadBuffer[15][40];
	char nameBuffer[15][40];
	char testreport[24];  
	char tempitem[40];
    
	
    //add for saving test report
    fp = open_read_file(TEST_REPORT_SAVE_FILE);  
	
	LOGD(TAG "=====create_report_open111=====");

	if(fp == NULL)  
	{
	    LOGD(TAG "report_open TEST_REPORT_SAVE_FILE is null");
	   create_report(item, rpt_items, maxitems, buf, size);
	}
	else
	{
	    LOGD(TAG "report_open TEST_REPORT_SAVE_FILE is not null");
       // read_test_report(rpt_items, fp); 
      // if ((fgets(ReadBuffer,15,fp))==NULL){LOGD(TAG "ReadBuffer110=%s\n", ReadBuffer);  } 
       while((fgets(ReadBuffer[readitemNum],40,fp))!=NULL) 
             {
                 int i=strlen(ReadBuffer[readitemNum]);                  
                  if(ReadBuffer[readitemNum][i-1]=='\n') //因为最后一行没有回车符
                  {
                    ReadBuffer[readitemNum][i-1]='\0'; 
					strncpy(nameBuffer[readitemNum],ReadBuffer[readitemNum],i-3);
					//strcpy(testreport[readitemNum],ReadBuffer[readitemNum][i-2]);   
				    testreport[readitemNum]=ReadBuffer[readitemNum][i-2];
                  }
			 LOGD(TAG "ReadBuffer[%d]=%s\n", readitemNum,ReadBuffer[readitemNum]); 
			 LOGD(TAG "testreport[%d]=%c\n", readitemNum,testreport[readitemNum]);  
			 readitemNum++;  
             }
    ...
    //add for saving test report   
					...
					if (mod && mod->visible && len < size) 
							{	
						  	for(itemNum=0;itemNum<maxitems;itemNum++)  

							{   
							sprintf(tempitem, "%s =", item->name); 
						    LOGD(TAG "ReadBuffer33[%d]---------------------%s\n", itemNum,ReadBuffer[itemNum]);  
							LOGD(TAG "ReadBuffer strlen--------------------%d\n", strlen(ReadBuffer[itemNum]));  
							LOGD(TAG "testreport[%d]-----------------------%c\n",itemNum,testreport[itemNum]);   
							LOGD(TAG "tempitem3333-------------------------%s\n", tempitem);  
						    LOGD(TAG "tempitem3333 strlen------------------%d\n", strlen(tempitem));      
						 // if(((strncmp(tempitem, ReadBuffer[itemNum], strlen(tempitem)))==0)&&((testreport[itemNum]=='O')||(testreport[itemNum]=='X')))     
							if((strncmp(tempitem, ReadBuffer[itemNum], strlen(tempitem)))==0)       
                            {   
							LOGD(TAG " item->nam----------------%s match sucesss!\n", item->name);    
							ptr = buf + len;
							len += sprintf(ptr, "[%c] %s ",testreport[itemNum], item->name);  
							//len += sprintf(ptr, "[%c] %s ",
							//(mod->test_result >= FTM_TEST_MAX) ?
							//result[FTM_TEST_UNKNOWN] : result[mod->test_result], item->name);
							ptr[len++] = '\0';
							rpt_items[i].id = mod->id;
							rpt_items[i].name = ptr;
							if(testreport[itemNum]=='O'){rpt_items[i].background = bgc[1];   LOGD(TAG "tempitem green============ \n" ); }
			               	else if(testreport[itemNum]=='X'){rpt_items[i].background = bgc[2];  LOGD(TAG "tempitem red========= \n" ); }
							else {rpt_items[i].background = bgc[0];  LOGD(TAG "tempitem red========= \n" ); }
						    //rpt_items[i].background = (mod->test_result >= FTM_TEST_MAX) ?
							//0 : bgc[mod->test_result];
							i++;
							break;  
...
#endif    
	LOGD(TAG "=====create_report_open444=====");
    if (i < maxitems - 1) {
        rpt_items[i].id   = ITEM_MAX_IDS;
        rpt_items[i].name = uistr_info_test_report_back;
    }
...
#if 0 //KK VERSION  this function have 
//zhangsh add for ATA
int write_barcode(int fd, char* barcode)
{
    const int BUF_SIZE = 128;
...
//zhangsh add
static int create_hyinfo(char *info,int size)
{
int fd=-1;
int fd2=-1;
int fd_dt = -1;
...
#if 1//def __HY_FACTORY__  
			if(strlen(barcode) <= 0)
			strcpy(barcode, "\n");
			else
			{
			strcpy(serialnum,barcode);
			  if((barcode[60]=='1')&&(barcode[61]=='0'))
			  	{
			  	  strcpy(barcode, "Pass");
				  cal_check=1; //zxs
			       } 
...
if(is_support_modem(5)){
	...
	if(strlen(barcode_dt) <= 0)
		...
		{
		  if((barcode_dt[60]=='1')&&(barcode_dt[61]=='0'))
		  	strcpy(barcode_dt, "Pass");
		  else
		  	strcpy(barcode_dt, "\n");
		}
...
static int item_test_report(item_t *items, char *buf, int bufsz)
{
	...
	#ifdef Factory_SaveReport
    num = create_report_open(items, rpt_items, ITEM_MAX_IDS, buf, bufsz);
    #else
...
{ 
    ...
    init_text(&info, buf, COLOR_YELLOW);
#ifdef __HY_FACTORY_CUST2__ 
    init_text(&swver, hyinfo, COLOR_YELLOW);
    create_hyinfo(hyinfo, 500);
#endif
  ...
  // if(cal_check==1)  //zxs 20140915               
	if(1)  //zxs 20140915    
     { 
     
    #if 0 //def __HY_FACTORY_CUST2__ 
	...
	fiv.m_selected++;
		...
		#ifdef __HY_FACTORY_CUST2__
		//bool needAuto=true;
        //int chosen_item = fiv.run(&fiv, &needAuto);
        fiv.set_text(&fiv,&swver);
		#endif
			int chosen_item = fiv.run(&fiv, NULL);
		#ifdef __HY_FACTORY_CUST2__
       	LOGD(TAG "in main while!\n");
		#endif
			switch (chosen_item) 
			{
			case ITEM_MUI_TEST:
